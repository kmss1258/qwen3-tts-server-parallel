...
from ..services.gpu_lane_dispatcher import GpuLaneDispatcher, LaneQueueFull
from qwen_tts.inference.qwen3_tts_model import VoiceClonePromptItem

logger = logging.getLogger(__name__)

_lane_dispatcher: Optional[GpuLaneDispatcher] = None
_lane_dispatcher_key: Optional[tuple] = None
...
def _resolve_lane_device_ids(backend) -> list[Optional[int]]:
...
def _get_lane_dispatcher(backend) -> GpuLaneDispatcher:
    global _lane_dispatcher
    global _lane_dispatcher_key

    device_ids = _resolve_lane_device_ids(backend)
    max_queue = int(os.getenv("TTS_LANE_MAX_QUEUE", "8"))
    key = (tuple(device_ids), max_queue)
    if _lane_dispatcher is None or _lane_dispatcher_key != key:
        _lane_dispatcher = GpuLaneDispatcher(
            device_ids=device_ids, max_queue_size=max_queue
        )
        _lane_dispatcher_key = key
        logger.info(
            "lane_dispatcher_config lanes=%s queue_limit=%s",
            len(device_ids),
            max_queue,
        )
    return _lane_dispatcher
...
async def generate_speech(
    ...
    dispatcher = _get_lane_dispatcher(backend)
    ...
    try:
        audio, sr = await dispatcher.submit(
...
async def list_voices():
    ...
    default_voices = [
        VoiceInfo(
            id="Vivian", name="Vivian", language="English", description="Female voice"
        ),
        VoiceInfo(id="Ryan", name="Ryan", language="English", description="Male voice"),
        VoiceInfo(
            id="Sophia", name="Sophia", language="English", description="Female voice"
        ),
        VoiceInfo(
            id="Isabella",
            name="Isabella",
        ...
        VoiceInfo(
            id="Lily", name="Lily", language="English", description="Female voice"
        ),
    ]
    ...
    openai_voices = [
        VoiceInfo(
            id="alloy",
            name="Alloy",
            description="OpenAI-compatible voice (maps to Vivian)",
        ),
        VoiceInfo(
            id="echo", name="Echo", description="OpenAI-compatible voice (maps to Ryan)"
        ),
        VoiceInfo(
            id="fable",
            name="Fable",
            description="OpenAI-compatible voice (maps to Sophia)",
        ),
        VoiceInfo(
            id="nova",
            name="Nova",
            description="OpenAI-compatible voice (maps to Isabella)",
        ),
        VoiceInfo(
            id="onyx", name="Onyx", description="OpenAI-compatible voice (maps to Evan)"
        ),
        VoiceInfo(
            id="shimmer",
            name="Shimmer",
            description="OpenAI-compatible voice (maps to Lily)",
        ),
    ]
...
async def get_voice_clone_capabilities():
    ...
    try:
        ...
        model_type = (
            backend.get_model_type()
            if hasattr(backend, "get_model_type")
            else "unknown"
        )
...
@router.get("/audio/voice-design/capabilities")
async def get_voice_design_capabilities():
    """
    Get voice design capabilities of the current backend.

    Returns whether voice design is supported and what model type is loaded.
    Voice design requires the VoiceDesign model (Qwen3-TTS-12Hz-1.7B-VoiceDesign).
    """
    try:
        backend = await get_tts_backend()

        supports_design = backend.supports_voice_design()
        model_type = (
            backend.get_voice_design_model_type()
            if hasattr(backend, "get_voice_design_model_type")
            else "unknown"
        )

        return VoiceDesignCapabilities(
            supported=supports_design,
            model_type=model_type,
        )

    except Exception as e:
        logger.warning(f"Could not get voice design capabilities: {e}")
        return VoiceDesignCapabilities(
            supported=False,
            model_type="unknown",
        )
...
async def create_voice_clone(
    ...
    try:
        ...
        dispatcher = _get_lane_dispatcher(backend)
        audio, sample_rate = await dispatcher.submit(
...
async def create_voice_clone_prompt(request: VoiceClonePromptRequest):
    ...
    try:
        ...
        dispatcher = _get_lane_dispatcher(backend)
        prompt_items = await dispatcher.submit(
            backend.create_voice_clone_prompt,
            ref_audio=ref_audio,
            ref_audio_sr=ref_sr,
            ref_text=request.ref_text,
            x_vector_only_mode=request.x_vector_only_mode,
...
@router.post("/audio/voice-clone/cache/clear")
async def clear_voice_clone_cache():
    """Clear the voice clone prompt cache (if enabled)."""
    backend = await get_tts_backend()
    cleared = 0
    try:
        cleared = backend.clear_voice_clone_cache()
    except Exception as e:
        logger.warning(f"Failed to clear voice clone cache: {e}")

    return {
        "cleared": cleared,
        "enabled": backend.voice_clone_cache_enabled(),
    }
...
async def create_voice_design(
    ...
    """
    Generate speech using a designed voice from natural-language instructions.
    ...
    try:
        ...
        if not backend.supports_voice_design():
            raise HTTPException(
                ...
                detail={
                    ...
                    "message": "Voice design requires the VoiceDesign model (Qwen3-TTS-12Hz-1.7B-VoiceDesign). "
                    "Configure the server with the VoiceDesign model and restart.",
                    "type": "invalid_request_error",
                },
            )
        ...
        return Response(
            ...
            media_type=content_type,
            headers={
                "Content-Disposition": f"attachment; filename=voice_design.{request.response_format}",
                "Cache-Control": "no-cache",
            },
        )
