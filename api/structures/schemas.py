...
class VoiceDesignRequest(BaseModel):
    ...
    input: str = Field(
        ...,
        description="The text to generate audio for using the designed voice.",
        max_length=4096,
    )
    instruct: Optional[str] = Field(
        default=None,
        description="Natural-language description of the desired voice/style.",
        max_length=4096,
    )
    language: Optional[str] = Field(
        default="Auto",
...
class VoiceDesignCapabilities(BaseModel):
    """Response schema for voice design capabilities endpoint."""

    supported: bool = Field(
        ...,
        description="Whether voice design is supported by the current backend.",
    )
    model_type: str = Field(
        ...,
        description="The type of model loaded (voice_design or other).",
    )
