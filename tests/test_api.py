...
class TestVoiceCloneEndpoints:
    ...
    def test_voice_clone_capabilities_endpoint_returns_valid_structure(
    ...
    def test_voice_clone_capabilities_custom_voice_not_supported(
        self, client, monkeypatch
    ):
...
class TestVoiceCloneCacheEndpoints:
    """Tests for voice clone cache endpoints."""

    def test_voice_clone_cache_clear_endpoint(self, client):
        """Test that cache clear endpoint returns expected structure."""
        from unittest.mock import MagicMock
        from api.backends import factory

        mock_backend = MagicMock()
        mock_backend.is_ready.return_value = True
        mock_backend.voice_clone_cache_enabled.return_value = True
        mock_backend.clear_voice_clone_cache.return_value = 3

        factory._backend_instance = mock_backend

        response = client.post("/v1/audio/voice-clone/cache/clear")
        assert response.status_code == 200
...
class TestVoiceDesignEndpoints:
    ...
    def test_voice_design_capabilities_endpoint_returns_valid_structure(
        self, client, monkeypatch
    ):
        """Test that voice design capabilities endpoint returns valid response structure."""
        from unittest.mock import MagicMock
        from api.backends import factory

        mock_backend = MagicMock()
        mock_backend.is_ready.return_value = True
        mock_backend.supports_voice_design.return_value = True
        mock_backend.get_voice_design_model_type.return_value = "voice_design"

        factory._backend_instance = mock_backend

        response = client.get("/v1/audio/voice-design/capabilities")
        assert response.status_code == 200
    ...
    def test_voice_design_requires_input(self, client):
        """Test that voice design endpoint requires input text."""
        response = client.post(
            "/v1/audio/voice-design",
            json={
                "instruct": "Bright, energetic adult voice",
            },
        )

        assert response.status_code == 422

    def test_voice_design_not_supported_returns_400(self, client, monkeypatch):
        """Test that voice design returns 400 error when not supported."""
        from unittest.mock import MagicMock
        from api.backends import factory

        mock_backend = MagicMock()
        mock_backend.is_ready.return_value = True
        mock_backend.supports_voice_design.return_value = False
        ...
        data = response.json()
        assert "detail" in data
        assert data["detail"]["error"] == "voice_design_not_supported"
