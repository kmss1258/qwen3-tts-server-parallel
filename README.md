...
- `POST /v1/audio/voice-clone` - Clone a voice from reference audio and synthesize speech
- `GET /v1/audio/voice-clone/capabilities` - Voice clone capability check
- `POST /v1/audio/voice-clone/cache/clear` - Clear voice clone prompt cache
- `POST /v1/audio/voice-design` - Generate speech from a voice design prompt
- `GET /v1/audio/voice-design/capabilities` - Voice design capability check
...
```bash
curl -X POST http://localhost:19160/v1/audio/voice-clone \
  -H "Content-Type: application/json" \
  -d '{
    "input": "Hello, this is a cloned voice sample.",
    "ref_audio": "<base64-audio>",
    "ref_text": "Hello, this is the reference audio transcript.",
...
```bash
curl -X POST http://localhost:19160/v1/audio/voice-design \
  -H "Content-Type: application/json" \
  -d '{
    "input": "Hello, this is a voice design sample.",
    "instruct": "Warm, confident adult female voice with a bright tone.",
    "language": "English",
    "response_format": "mp3",
    "speed": 1.0
  }'
...
Configure prompt cache size/TTL (server-side):

```bash
TTS_VOICE_CLONE_CACHE_SIZE=32
TTS_VOICE_CLONE_CACHE_TTL=0
```

Clear cache manually:

```bash
curl -X POST http://localhost:19160/v1/audio/voice-clone/cache/clear
```

### Multi-Model Mode (Base + VoiceDesign)

To run voice cloning and voice design from a single server/port, enable multi-model mode:
...
### Multi-GPU Multi-Model Mode

Load Base + VoiceDesign on each GPU and round-robin requests:

```bash
TTS_MULTI_MODEL=true
TTS_MULTI_GPU=true
TTS_GPU_DEVICE_IDS=0,1
TTS_SPEECH_MODEL_NAME=none
TTS_BASE_MODEL_NAME=Qwen/Qwen3-TTS-12Hz-1.7B-Base
TTS_VOICE_DESIGN_MODEL_NAME=Qwen/Qwen3-TTS-12Hz-1.7B-VoiceDesign
```
